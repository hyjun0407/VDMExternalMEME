...
namespace voyager
{
	...
	template <class T>
	auto rpm_buffer(guest_phys_t dirbase, guest_virt_t virt_addr, long size) -> T*
	{
		T* buffer = new T[size + 1L];
		memset(buffer, size + 1L, 0);
		auto result = voyager::copy_virt(dirbase, virt_addr,
			voyager::current_dirbase(), (voyager::guest_virt_t)buffer, size);

		if (result != voyager::vmxroot_error_t::error_success) {
			memset(buffer, size + 1L, 0);
		}
		return buffer;
	}
	...
	{
		auto result = copy_virt(current_dirbase(), (guest_virt_t)&data,
